...
# Settings
NUM_THREADS = min(8, max(1, os.cpu_count() - 1))  # number of Xception multiprocessing threads
...
class Profile(contextlib.ContextDecorator):
    ...
    def __init__(self, t=0.0, device: torch.device = None):
        """Initializes a profiling context for Xception with optional timing threshold and device specification."""
        self.t = t
        self.device = device
...
def check_git_status(repo="ultralytics/yolov5", branch="master"):
    """Checks if Xception code is up-to-date with the repository, advising 'git pull' if behind; errors return informative
    messages.
    """
    ...
    if n > 0:
        ...
        s += f"⚠️ Xception is out of date by {n} commit{'s' * (n > 1)}. Use '{pull}' or 'git clone {url}' to update."
    else:
...
def check_git_info(path="."):
    """Checks Xception git info, returning a dict with remote URL, branch name, and commit hash."""
...
def non_max_suppression(
    ...
    if isinstance(prediction, (list, tuple)):  # Xception model in validation model, output = (inference_out, loss_out)
...
def print_mutation(keys, results, hyp, save_dir, bucket, prefix=colorstr("evolve: ")):
    ...
    with open(evolve_yaml, "w") as f:
        ...
        f.write(
            "# Xception Hyperparameter Evolution Results\n"
            + f"# Best generation: {i}\n"
